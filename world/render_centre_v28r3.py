...
HTML = PREFIX+'.html'
CANDIDATE = 'continuum-v28r3-candidate.js'
assert not list(HERE.glob(PREFIX+'*')), 'Preserve one-shot evidence'
assert json.loads((HERE/'centre-candidate-v28r3-final-check.json').read_text())['integrity_passed']
...
html = html.replace('A reported-pattern interpretation', 'ISOLATED v28r3 · synthesised diagnostic')
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for label in ['entry','deep']:
                if label=='deep':
                    mark('real walk to z -6 and pause')
                    page.locator('#pause').click()
