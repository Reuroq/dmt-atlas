...
OUT=HERE/'centre-v34-first-root-check.json'
assert not OUT.exists()
def digest(name):
    return hashlib.sha256((HERE/name).read_bytes()).hexdigest()
def main():
    baseline=json.loads((HERE/'numeric-candidate-v34-check.json').read_text())
    assert baseline['passed'] and not baseline['errors']
    assert all(digest(n)==h for n,h in baseline['files'].items())
    bounds=json.loads((HERE/'centre-v34-bounds-check.json').read_text())
    assert bounds['passed'] and all(digest(n)==h for n,h in bounds['source_hashes'].items())
    cases=json.loads((HERE/'numeric-candidate-v34-raw.json').read_text())
    ...
    files=['numeric-candidate-v34-check.json','numeric-candidate-v34-raw.json',
        'centre-v34-bounds-check.json',
