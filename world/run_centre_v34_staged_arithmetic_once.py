...
import subprocess
import sys
from probe_centre_v34_staged_arithmetic import HERE, PREFIX, digest, gate, save
...
def main(label):
    ...
    for suffix in ('-' + label + '-once-launch.json', '-' + label + '-once-exit.json', '-' + label + '-once.log',
        ...
        assert not (HERE / (PREFIX + suffix)).exists(), 'Never replay'
    script = {'static': 'check_centre_v34_staged_arithmetic.py', 'close': 'close_centre_v34_staged_arithmetic_preparation.py',
              'runtime': 'probe_centre_v34_staged_arithmetic.py'}[label]
...
PathName = 'run_centre_v34_staged_arithmetic_once.py'
