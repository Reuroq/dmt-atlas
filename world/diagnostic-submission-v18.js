...
// Isolated diagnostic: readPixels is queued before the application's fence.
...
function frame(now){
 ...
 if(renderFence){
  ...
  if(gpuReady){probeEvent('fence-retired');retirePixels();renderGL.deleteSync(renderFence);renderFence=null;}
 }
 if(drawInvalidated&&gpuReady){
  probeEvent('submit-before');composite.render(scene,camera,['onset','afterglow'].includes(stage.id)?.16:stage.id==='void'?.3:.8,animTime,grain+seam*.25,seam);renderReady=true;frames++;
