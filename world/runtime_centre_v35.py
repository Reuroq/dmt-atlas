"""One-shot v35 prerequisite checks and durable failure receipts."""
import hashlib
import json
from pathlib import Path
from cost_lifecycle_v35 import atomic_json, install_signal_receipts, restore_signals
...
def one_shot(label, callback):
    start = HERE/f'centre-v35-{label}-started.json'
    failure = HERE/f'centre-v35-{label}-failure.json'
    assert not start.exists() and not failure.exists(), 'Preserve partial attempt; no replay'
    fixture_gate()
    atomic_json(start, {'started': True, 'fixture_manifest': digest('centre-v35-fixture-integrity.json')})
