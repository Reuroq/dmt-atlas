...
import numpy as np
from field_centre_v31 import PIXEL,parts

HERE=Path(__file__).resolve().parent
OUT=HERE/'centre-v31-first-root-check.json'
assert not OUT.exists()
def digest(name):
    return hashlib.sha256((HERE/name).read_bytes()).hexdigest()
baseline=json.loads((HERE/'numeric-candidate-v31-check.json').read_text())
assert all(digest(n)==h for n,h in baseline['files'].items())
bounds=json.loads((HERE/'centre-v31-bounds-check.json').read_text())
assert bounds['passed'] and all(digest(n)==h for n,h in bounds['source_hashes'].items())
cases=json.loads((HERE/'numeric-candidate-v31-raw.json').read_text())
M=140+140*PIXEL+170*PIXEL**2

def composed(v):
    f,a,b=np.moveaxis(v,-1,0)
    return np.minimum(np.minimum(np.maximum(f,np.maximum(abs(a)-.065,.16-abs(b))),
        np.maximum(f+.45,np.maximum(abs(a)-.18,abs(b)-.03))),f+12)
...
def lower_bound(left,right,width):
    ...
    return np.minimum(np.minimum(np.maximum(fl,np.maximum(al-.065,.16-bu)),
        np.maximum(fl+.45,np.maximum(al-.18,bl-.03))),fl+12)
...
for case,old in zip(cases,baseline['cases']):
    ...
    for reference in old['references']:
        ...
        envelope_free=np.maximum(rho[:-1],rho[1:])+.003<4.9
...
files=['numeric-candidate-v31-check.json','numeric-candidate-v31-raw.json',
    'centre-v31-bounds-check.json',
    'field_centre_v31.py',Path(__file__).name]
