...
from verify import ARGS, open_page, diag, stage, next_stage, hold_until, results, evidence
...
with sync_playwright() as p:
    ...
    assert 'Localized lime, cyan, yellow and pink highlights' in table
    assert 'Credit is limited to visible darkness' in table
    page.locator('#closeEvidence').click()
    out['checks'].append('Fresh v23 Sources table contains reviewed luminous and dark/black observations')
    ...
    browser.close()
(HERE / 'verification-continuum-v23.json').write_text(json.dumps(out, indent=2) + '\n', encoding='utf-8')
