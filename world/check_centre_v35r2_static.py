"""Frozen v35r2 static closure: exact derivations, actual JS binding paths, no GPU."""
import ast
import hashlib
import json
import re
...
from build_centre_v35r2 import COPIES, EDITS, CONTROL
...
def main():
    ...
    control_delta = p.read('centre-v35-static-r2-source-delta.json')
    amendments = p.read('centre-v35r2-prefreeze-amendments.json')
    for name, entry in control_delta.items():
        assert p.digest(entry['base']) == entry['base_sha256']
        source = (p.HERE / entry['base']).read_text().replace(*entry['rename'])
        ...
        if name in amendments:
            ...
            assert amendment['before_sha256'] == entry['sha256']
            for before, after in amendment['edits']:
                assert source.count(before) == 1
                source = source.replace(before, after, 1)
            assert hashlib.sha256(source.encode()).hexdigest() == amendment['after_sha256']
        assert source == (p.HERE / name).read_text(), name
    delta = p.read(p.PREFIX + '-source-delta.json')
    assert list(delta) == [n.replace('v34', 'v35r2') for n in COPIES]
    for name, entry in delta.items():
        assert p.digest(entry['base']) == entry['base_sha256']
        source = (p.HERE / entry['base']).read_text().replace('v34', 'v35r2')
        for before, after in entry['edits']:
            assert source.count(before) == 1
            source = source.replace(before, after, 1)
        ...
        elif name != 'runtime_centre_v35r2.py':
    ...
    candidate = (p.HERE / 'continuum-v35r2-candidate.js').read_text()
    reference = (p.HERE / 'continuum-v35r2-unshared-reference.js').read_text()
    ...
    for stem in ('candidate', 'unshared-reference'):
        durable_node('syntax-' + stem, ['--check', str(p.HERE / ('continuum-v35r2-' + stem + '.js'))])
    durable_node('syntax-harness', ['--check', str(p.HERE / 'check_centre_v35r2_uniforms.cjs')])
    fixtures = ['gpu_probe_v35r2_candidate.html', 'gpu_probe_v35r2_leaves.html',
                'gpu_cost_v35r2_candidate.html', 'gpu_cost_v35r2_parent.html']
    snapshots = {}
    for name in fixtures:
        ...
        expected = 'continuum-v35r2-unshared-reference.js' if name.endswith('parent.html') else 'continuum-v35r2-candidate.js'
        assert dependencies == ['vendor/three.min.js', expected]
        result = durable_node('bindings-' + name, ['check_centre_v35r2_uniforms.cjs', expected, name])
        snapshots[name] = json.loads(result)
    for stem in ('candidate', 'unshared-reference'):
        name = 'continuum-v35r2-' + stem + '.js'
        snapshots[name] = json.loads(durable_node('bindings-' + stem, ['check_centre_v35r2_uniforms.cjs', name]))
    ...
    render = (p.HERE / 'render_centre_v35r2.py').read_text()
    assert render.replace('v35r2', 'v34') == (p.HERE / 'render_centre_v34.py').read_text()
    tree = ast.parse((p.HERE / 'probe_numeric_candidate_v35r2.py').read_text())
