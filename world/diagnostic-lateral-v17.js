...
const lateralTrace=window.lateralTrace=[];
...
function syncMovement(){
 ...
 if(entered&&!paused&&!transition&&!$('evidence').open&&!document.hidden){
  traceLateral('move-before',{dt});move(dt);traceLateral('move-after',{dt});if(!camera.position.equals(drawnPosition))drawInvalidated=true;
 }
}
function move(dt){
...
addEventListener('keydown',e=>{
 ...
 if(['a','d'].includes(e.key))traceLateral('native-before',{event:e.type,stamp:e.timeStamp});const key=e.key.length===1?e.key.toLowerCase():e.key;if(keyMoves[key]&&entered){e.preventDefault();syncMovement();if(!paused)input.add(keyMoves[key]);}
...
addEventListener('keyup',e=>{if(['a','d'].includes(e.key))traceLateral('native-before',{event:e.type,stamp:e.timeStamp});syncMovement();input.delete(keyMoves[e.key.length===1?e.key.toLowerCase():e.key]);});
...
$('world').addEventListener('pointerup',e=>{if(drag&&Math.hypot(e.clientX-drag.startX,e.clientY-drag.startY)<7){const ray=new T.Raycaster();ray.setFromCamera(new T.Vector2(e.clientX/innerWidth*2-1,1-e.clientY/innerHeight*2),camera);const hit=ray.intersectObjects(actors.map(a=>a.g),true)[0];if(hit){let o=hit.object;while(o&&!o.userData.entityKey)o=o.parent;const a=actors.find(a=>a.g===o);if(a){a.engaged=true;interactionCount++;drawInvalidated=true;$('sceneHint').textContent=a.kind==='mantis'?'The examiner inclines its head and extends its forelimbs.':a.kind==='mother'?'The presence turns toward you, her arms held open.':'The figure turns toward you and lifts its changing forms.';}}}drag=null;});
$('world').addEventListener('pointercancel',()=>{drag=null;});
document.querySelectorAll('[data-move]').forEach(b=>{b.addEventListener('pointerdown',e=>{e.preventDefault();if(paused)return;syncMovement();b.setPointerCapture(e.pointerId);input.add(b.dataset.move);});for(const name of ['pointerup','pointercancel','lostpointercapture'])b.addEventListener(name,()=>{syncMovement();input.delete(b.dataset.move);});});
...
function frame(now){
 requestAnimationFrame(frame);traceLateral('raf',{raf:now});const dt=Math.min((now-lastFrame)/1000,1);lastFrame=now;
 syncMovement();
 ...
 if(drawInvalidated&&gpuReady){
  traceLateral('submit-before');composite.render(scene,camera,['onset','afterglow'].includes(stage.id)?.16:stage.id==='void'?.3:.8,animTime,grain+seam*.25,seam);renderReady=true;frames++;traceLateral('submit-after');
