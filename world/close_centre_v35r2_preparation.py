...
NEXT = (
    'Verify centre-v35r2-fixture-integrity.json files/protected/display and '
    'centre-v35r2-close-once actual-exit0 receipt hashes. Static v35r2 uniform-offset '
    ...
    'certification, never nearest-root substitution. Do not edit/replay frozen v34/v35r2 '
    'fixtures or relax exact jets, tolerances, budgets, cost, capture or acceptance gates.'
)
STATUS = (
    'Isolated v35r2 uniform-offset preparation CLOSED static PASS; no field/browser runtime. '
    ...
    'not v35r2. Live/defaults/ledgers unchanged; all19/source/coverage/route/full acceptance '
...
def main():
    ...
    for pattern in ('centre-v35r2-*-started.json', 'run-centre-v35r2-*-once-launch.json',
                    'numeric-candidate-v35r2-*.json', 'diagnostic-centre-v35r2*'):
        assert not list(p.HERE.glob(pattern)), pattern
    assert not (p.HERE / 'centre-v35r2-integrity.json').exists()
    review = '''# Isolated v35r2 preparation — CLOSED static PASS, no field runtime
...
'''
    ...
    for name in ('NOTES.md', 'README.md', 'status.txt'):
        archive = name + '-before-centre-v35r2-preparation'
...
- Scalar oracle/Hessian/CSG,48cases/75996rays,exact expanded/composite/normal/cheap-full jets,720fixed+720independent roots,12balanced cost pairs and original renderer unchanged. Runtime gate additionally requires actual preparation closure receipts. Frozen v34/v35r2 evidence immutable. No field/browser/root/cost/capture/acceptance run,new image/inspection or promotion.
- Existing four local CPU-profile crossing pairs inside old .003 cells guide a DISTINCT additive root-isolation fixture next; no root algorithm changed here, no earliest-root proof. Original exact-jet/716-of-720 failures remain. centre-v35r2-fixture-integrity.json binds preparation/historical/protected/current display. Prior display archived; NOTES once/BOM retained. latest unchanged inspected REJECTED GAME v33r6;all19 unfinished.
...
''' + NEXT + '\n'
    ...
    for path in p.HERE.glob(p.PREFIX + '-*'):
        if path.is_file() and path.name != 'centre-v35r2-close-once.log':
    ...
    print('Saved-only v35r2 preparation CLOSED; no numerical or visual pass')
