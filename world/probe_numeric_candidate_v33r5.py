...
from field_centre_v33r5 import field, parts, compose_jets, lower_parts, cheap_parts

HERE = Path(__file__).resolve().parent

def main():
    destination = HERE/'numeric-candidate-v33r5-check.json'
    assert not destination.exists(), 'One-shot probe already recorded'
    started = HERE/'centre-v33r5-gpu-started.json'
    ...
    with sync_playwright() as p:
        ...
        page.goto((HERE/'gpu_probe_v33r5_candidate.html').as_uri())
        ...
        leaf_page.goto((HERE/'gpu_probe_v33r5_leaves.html').as_uri())
        ...
        browser.close()
    (HERE/'numeric-candidate-v33r5-raw.json').write_text(json.dumps(cases)+'\n')
    ...
    out = {'method':'New v33r5 actual float GLSL rays plus all smooth-leaf jets at saved actual roots. Independent float64 scalar/leaf FD; exact float32 CSG selection and original normal replay. Composite FD disagreements retained at creases, not a unique-normal assertion. Original six plus central3x3 dense .003 first-crossing references, even/odd grids, HIGH/LOW, entry/deep, times2.4077/3.5242/4/4.895/8/221/900. Sampled evidence only; no universal convergence, filtering, realism or performance proof.',
           'limits':{'gradient':.005,'bound_ratio':1.001,'scalar':.001,'root':.03},
           'files':{name:digest(HERE/name) for name in ['continuum.js','trip.js','continuum-v25.js','continuum-v33r5-candidate.js','build_centre_v33r5.py','centre-v33r5-identifier-proof.json','centre-v33r5-bounds-check.json','centre-v33r5-gpu-started.json','gpu_probe_v33r5_leaves.html','gpu_probe_v33r5_candidate.html','probe_numeric_candidate_v33r5.py','field_centre_v33r5.py','numeric-candidate-v33r5-raw.json']},
