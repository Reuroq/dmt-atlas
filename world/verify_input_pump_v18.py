...
assert not (HERE/'verification-input-pump-v18.json').exists(), 'Preserve evidence'
def open_page(browser, **options):
    ...
    page.goto((HERE/'input-pump-v18-candidate.html').as_uri(),wait_until='load')
...
out = {'render_signature': render_signature(), 'checks': [], 'passed': False,
       'exit_walk_timeout_ms': args.walk_timeout_ms, 'candidate_sha256': __import__('hashlib').sha256((HERE/'trip-v18-candidate.js').read_bytes()).hexdigest()}
with sync_playwright() as p:
    ...
    try:
        ...
        # Preserve the original centre tolerance; navigation now has an independent pump.
        # This is a walking check, not a high-frame-rate assumption; restart below resets pose.
        out['centre_return_x']=diag(page)['position'][0]
        ...
        browser.close()
(HERE / 'verification-input-pump-v18.json').write_text(json.dumps(out, indent=2) + '\n', encoding='utf-8')
