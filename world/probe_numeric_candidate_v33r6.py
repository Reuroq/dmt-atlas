...
from field_centre_v33r6 import field, parts, compose_jets, lower_parts, cheap_parts

HERE = Path(__file__).resolve().parent

def main():
    destination = HERE/'numeric-candidate-v33r6-check.json'
    assert not destination.exists(), 'One-shot probe already recorded'
    started = HERE/'centre-v33r6-gpu-started.json'
    assert not started.exists(), 'Do not replay a partial GPU run'
    gate = json.loads((HERE/'centre-v33r6-implementation.json').read_text())
    ...
    started.write_text(json.dumps({'started':True,'implementation_sha256':digest(HERE/'centre-v33r6-implementation.json')})+'\n')
    errors, cases = [], []
    with sync_playwright() as p:
        ...
        page.goto((HERE/'gpu_probe_v33r6_candidate.html').as_uri())
        ...
        leaf_page.goto((HERE/'gpu_probe_v33r6_leaves.html').as_uri())
        ...
        browser.close()
    (HERE/'numeric-candidate-v33r6-raw.json').write_text(json.dumps(cases)+'\n')
    ...
    out = {'method':'New v33r6 actual float GLSL rays plus all smooth-leaf jets at saved actual roots. Independent float64 scalar/leaf FD; exact float32 CSG selection and original normal replay. Composite FD disagreements retained at creases, not a unique-normal assertion. Original six plus central3x3 dense .003 first-crossing references, even/odd grids, HIGH/LOW, entry/deep, times2.4077/3.5242/4/4.895/8/221/900. Sampled evidence only; no universal convergence, filtering, realism or performance proof.',
           'limits':{'gradient':.005,'bound_ratio':1.001,'scalar':.001,'root':.03},
           'files':{name:digest(HERE/name) for name in ['continuum.js','trip.js','continuum-v25.js','continuum-v33r6-candidate.js','build_centre_v33r6.py','centre-v33r6-factoring-proof.json','centre-v33r6-bounds-check.json','centre-v33r6-gpu-started.json','gpu_probe_v33r6_leaves.html','gpu_probe_v33r6_candidate.html','probe_numeric_candidate_v33r6.py','field_centre_v33r6.py','numeric-candidate-v33r6-raw.json']},
    ...
    out['passed'] = not errors and all(s['passed'] for s in summaries) and len(root_errors)==48*15
...
if __name__ == '__main__':
    try:
        ...
        failure_path = HERE/'centre-v33r6-gpu-failure.json'
