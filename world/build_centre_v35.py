...
CONTROL = ['v35_preparation.py', 'build_centre_v35.py', 'check_centre_v35_static.py',
           'check_centre_v35_uniforms.cjs', 'run_centre_v35_preparation_once.py',
           'close_centre_v35_preparation.py']
...
def main():
    ...
    delta = {}
    for base in COPIES:
        name = base.replace('v34', 'v35')
        text = (p.HERE / base).read_text().replace('v34', 'v35')
        edits = EDITS if name.startswith('continuum-') else []
        if name == 'runtime_centre_v35.py':
            edits = [('def fixture_gate():\n',
                      'def fixture_gate():\n    from v35_preparation import closed\n    closed()\n')]
    ...
    sources = list(delta) + CONTROL + [p.PREFIX + '-source-delta.json']
    ...
    print('v35 isolated candidate and 18 derived sources frozen; no runtime')
