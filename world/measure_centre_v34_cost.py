...
from cost_lifecycle_v34 import Lifecycle, atomic_json, install_signal_receipts, restore_signals

HERE = Path(__file__).resolve().parent
OUT = HERE / 'centre-v34-cost-review.json'
RAW = HERE / 'centre-v34-cost-raw.json'
...
for name, key in [('centre-v34-bounds-check.json', 'source_hashes'),
                  ('numeric-candidate-v34-check.json', 'files'),
                  ('centre-v34-first-root-check.json', 'source_hashes')]:
    receipt = read(name)
...
files = ['continuum-v34-unshared-reference.js', 'continuum-v34-candidate.js',
         'gpu_cost_v34_parent.html', 'gpu_cost_v34_candidate.html',
         'centre-v34-fixture-integrity.json', 'centre-v34-bounds-check.json',
         'numeric-candidate-v34-check.json', 'centre-v34-first-root-check.json',
         'cost_lifecycle_v34.py', 'runtime_centre_v34.py', Path(__file__).name]
source_hashes = {n: digest(n) for n in files}
pairs, errors = [], []
failure = None
limits = ('Identical NEW v34 geometry/march; parent means unshared reference, candidate means retained common jets. Not a v34-v33r6 speedup. New balanced-order 12-pair sparse SwiftShader experiment. Same instrumentation and JS timing boundaries; '
...
journal = Lifecycle(HERE / 'centre-v34-cost-lifecycle')
...
try:
    with sync_playwright() as p:
        ...
        for arm in ['parent', 'candidate']:
            ...
            page.goto((HERE / f'gpu_cost_v34_{arm}.html').as_uri())
            pages[arm] = page
