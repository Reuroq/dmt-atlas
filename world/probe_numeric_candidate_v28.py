...
def main():
    destination = HERE/'numeric-candidate-v28-check.json'
    ...
    with sync_playwright() as p:
        ...
        page.goto((HERE/'gpu_probe_v28_candidate.html').as_uri())
        ...
        browser.close()
    (HERE/'numeric-candidate-v28-raw.json').write_text(json.dumps(cases)+'\n')
    ...
    out = {'method':'Actual float GLSL fields, analytic gradients and .4 bounds versus independent float64 field/finite differences. Original six plus central 3x3 .003 first-sign-crossing scans. Even legacy grids and odd exact-axis grids. Late times 221/900s. Sparse numerical evidence only; no universal root, precision, performance or realism proof.',
           ...
           'files':{name:digest(HERE/name) for name in ['continuum.js','trip.js','continuum-v25.js','continuum-v28-candidate.js','build_centre_candidate_v28.py','gpu_probe_v28_candidate.html','probe_numeric_candidate_v28.py','field_centre_v28.py','numeric-candidate-v28-raw.json']},
