...
vec3 preciseCos(vec3 x){return vec3(preciseCos(x.x),preciseCos(x.y),preciseCos(x.z));}
   ...
   vec4 compose(vec4 f,vec4 a,vec4 b,out float layer){
    ...
    vec4 petals=jmax(f,jmax(offset(aa,-.10),offset(-bb,.035)));
    vec4 branches=jmax(offset(f,.38),jmax(offset(aa,-.25),offset(bb,-.014)));
    vec4 backing=offset(f,10.);
   ...
   float raySlope(vec3 p,vec3 rd){return 16.+8.*pixelScale;}
   vec3 leafCurvature(){
    float s=pixelScale;
    return vec3(7.,305.,87.)+vec3(1.,190.,65.)*s+vec3(1.,110.,54.)*s*s;
   }
   ...
   vec4 traceSample(vec3 p,vec3 rd,out float safeStep){
    ...
    float petals=max(positiveReach(f,rd,m.x),max(positiveReach(offset(aa,-.10),rd,m.y),excludedBandReach(b,.035,rd,m.z)));
    float branches=max(positiveReach(offset(f,.38),rd,m.x),max(positiveReach(offset(aa,-.25),rd,m.y),positiveReach(offset(bb,-.014),rd,m.z)));
    safeStep=min(min(petals,branches),positiveReach(offset(f,10.),rd,m.x));
