...
from cost_lifecycle_v35 import Lifecycle, atomic_json, install_signal_receipts, restore_signals

HERE = Path(__file__).resolve().parent
OUT = HERE / 'centre-v35-cost-review.json'
RAW = HERE / 'centre-v35-cost-raw.json'
...
for name, key in [('centre-v35-bounds-check.json', 'source_hashes'),
                  ('numeric-candidate-v35-check.json', 'files'),
                  ('centre-v35-first-root-check.json', 'source_hashes')]:
    receipt = read(name)
    assert receipt['passed'] and all(digest(n) == h for n, h in receipt[key].items())

from runtime_centre_v35 import fixture_gate
...
files = ['continuum-v35-unshared-reference.js', 'continuum-v35-candidate.js',
         'gpu_cost_v35_parent.html', 'gpu_cost_v35_candidate.html',
         'centre-v35-fixture-integrity.json', 'centre-v35-bounds-check.json',
         'numeric-candidate-v35-check.json', 'centre-v35-first-root-check.json',
         'cost_lifecycle_v35.py', 'runtime_centre_v35.py', Path(__file__).name]
source_hashes = {n: digest(n) for n in files}
pairs, errors = [], []
failure = None
limits = ('Identical NEW v35 geometry/march; parent means unshared reference, candidate means retained common jets. Not a v35-v33r6 speedup. New balanced-order 12-pair sparse SwiftShader experiment. Same instrumentation and JS timing boundaries; '
...
journal = Lifecycle(HERE / 'centre-v35-cost-lifecycle')
...
try:
    with sync_playwright() as p:
        ...
        for arm in ['parent', 'candidate']:
            ...
            page.goto((HERE / f'gpu_cost_v35_{arm}.html').as_uri())
            pages[arm] = page
