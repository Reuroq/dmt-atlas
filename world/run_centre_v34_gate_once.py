...
from runtime_centre_v34 import fixture_gate, passed, digest

HERE = Path(__file__).resolve().parent
GATES = {
    'bounds': ('check_centre_v34_bounds.py', []),
    'gpu': ('probe_numeric_candidate_v34.py', [('centre-v34-bounds-check.json', 'source_hashes')]),
    'roots': ('audit_centre_v34_first_roots.py', [('numeric-candidate-v34-check.json', 'files')]),
    'cost': ('measure_centre_v34_cost.py', [('centre-v34-first-root-check.json', 'source_hashes')]),
}
...
def main():
    ...
    for prior in list(GATES)[:list(GATES).index(label)]:
        receipt = json.loads((HERE/f'run-centre-v34-{prior}-once-exit.json').read_text())
        assert receipt['actual_exit'] == 0, prior
        assert receipt['log_sha256'] == digest(f'run-centre-v34-{prior}-once.log'), prior
        assert receipt['launch_sha256'] == digest(f'run-centre-v34-{prior}-once-launch.json'), prior
        previous_launch = json.loads((HERE/f'run-centre-v34-{prior}-once-launch.json').read_text())
        assert previous_launch['fixture_manifest'] == digest('centre-v34-fixture-integrity.json'), prior
    log = HERE/f'run-centre-v34-{label}-once.log'
    exit_path = HERE/f'run-centre-v34-{label}-once-exit.json'
    launch = HERE/f'run-centre-v34-{label}-once-launch.json'
    ...
    atomic_json(launch, {'started': True, 'script': script, 'script_sha256': digest(script),
                         'fixture_manifest': digest('centre-v34-fixture-integrity.json')})
