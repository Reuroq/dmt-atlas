...
script='close_centre_v34_staged_arithmetic_r3_runtime.py'
...
p.save(stem+'-exit.json',{'actual_exit':actual,'launcher_error':error,'log_sha256':p.digest(stem+'.log'),
    'launch_sha256':p.digest(stem+'-launch.json'),'manifest_sha256':p.digest(manifest) if (p.HERE/manifest).exists() else None})
