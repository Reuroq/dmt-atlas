...
const presentationTrace=window.presentationTrace=[];
...
function frame(now){
 ...
 if(renderFence){
  ...
  if(gpuReady){tracePresentation('fence-retired');renderGL.deleteSync(renderFence);renderFence=null;}
 }
 if(drawInvalidated&&gpuReady){
  tracePresentation('submit-before');composite.render(scene,camera,['onset','afterglow'].includes(stage.id)?.16:stage.id==='void'?.3:.8,animTime,grain+seam*.25,seam);renderReady=true;frames++;tracePresentation('submit-after');
