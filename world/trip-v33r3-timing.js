...
let elapsed=0, animTime=0, stageAnimStart=0, yaw=0, pitch=0, transition=null, renderReady=false, frames=0;/*TIMING_INSERT_0_BEGIN*/
...
function build(s){
 drawInvalidated=true/*TIMING_INSERT_6_BEGIN*/,timingInvalidation('scene-build')/*TIMING_INSERT_6_END*/;
...
function syncMovement(){
 ...
 if(entered&&!paused&&!transition&&!$('evidence').open&&!document.hidden){
  move(dt);if(!camera.position.equals(drawnPosition))drawInvalidated=true/*TIMING_INSERT_5_BEGIN*/,timingInvalidation('movement')/*TIMING_INSERT_5_END*/;
 }
}
function move(dt){
...
$('world').addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId||paused)return;syncMovement();yaw-=(e.clientX-drag.x)*.0035;pitch=T.MathUtils.clamp(pitch-(e.clientY-drag.y)*.003,-1.25,1.25);drag.x=e.clientX;drag.y=e.clientY;camera.rotation.set(pitch,yaw,0);if(!camera.quaternion.equals(drawnRotation))drawInvalidated=true/*TIMING_INSERT_4_BEGIN*/,timingInvalidation('look')/*TIMING_INSERT_4_END*/;});
$('world').addEventListener('pointerup',e=>{if(drag&&Math.hypot(e.clientX-drag.startX,e.clientY-drag.startY)<7){const ray=new T.Raycaster();ray.setFromCamera(new T.Vector2(e.clientX/innerWidth*2-1,1-e.clientY/innerHeight*2),camera);const hit=ray.intersectObjects(actors.map(a=>a.g),true)[0];if(hit){let o=hit.object;while(o&&!o.userData.entityKey)o=o.parent;const a=actors.find(a=>a.g===o);if(a){a.engaged=true;interactionCount++;drawInvalidated=true/*TIMING_INSERT_3_BEGIN*/,timingInvalidation('engagement')/*TIMING_INSERT_3_END*/;$('sceneHint').textContent=a.kind==='mantis'?'The examiner inclines its head and extends its forelimbs.':a.kind==='mother'?'The presence turns toward you, her arms held open.':'The figure turns toward you and lifts its changing forms.';}}}drag=null;});
...
function resize(){camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);composite.resize();drawInvalidated=true/*TIMING_INSERT_2_BEGIN*/,timingInvalidation('resize')/*TIMING_INSERT_2_END*/;}
...
function frame(now){/*TIMING_INSERT_7_BEGIN*/
 timingRecord('raf:start',{},true);/*TIMING_INSERT_7_END*/
 ...
 if(!frozen||!lastDrawFrozen||!camera.position.equals(drawnPosition)||!camera.quaternion.equals(drawnRotation)||interactionCount!==drawnInteractions)drawInvalidated=true/*TIMING_INSERT_1_BEGIN*/,timingInvalidation('frame-policy')/*TIMING_INSERT_1_END*/;
 ...
 let gpuReady=true;/*TIMING_INSERT_8_BEGIN*/
 timingRecord('render-decision',{frozen,lastDrawFrozen},true);/*TIMING_INSERT_8_END*/
 if(renderFence){/*TIMING_INSERT_9_BEGIN*/
  timingSamplePoll=performance.now()-(timingLast['fence-poll:begin']??-Infinity)>=1000; if(timingSamplePoll)timingBegin('fence-poll');/*TIMING_INSERT_9_END*/
  gpuReady=/*TIMING_INSERT_10_BEGIN*/(timingWaitResult=/*TIMING_INSERT_10_END*/renderGL.clientWaitSync(renderFence,0,0)/*TIMING_INSERT_11_BEGIN*/)/*TIMING_INSERT_11_END*/!==renderGL.TIMEOUT_EXPIRED;/*TIMING_INSERT_12_BEGIN*/
  timingCounts['fence-result:'+timingWaitResult]=(timingCounts['fence-result:'+timingWaitResult]||0)+1; if(timingSamplePoll)timingEnd('fence-poll',{result:timingWaitResult,gpuReady});/*TIMING_INSERT_12_END*/
  if(gpuReady){renderGL.deleteSync(renderFence);renderFence=null;/*TIMING_INSERT_13_BEGIN*/timingRecord('fence-retired',{result:timingWaitResult,ageMs:timingOwner?performance.now()-timingOwner.createdMs:null});timingOwner=null;/*TIMING_INSERT_13_END*/}
 }
 if(drawInvalidated&&gpuReady){/*TIMING_INSERT_14_BEGIN*/
  timingSubmit++;timingBegin('submission');/*TIMING_INSERT_14_END*/
  composite.render(scene,camera,['onset','afterglow'].includes(stage.id)?.16:stage.id==='void'?.3:.8,animTime,grain+seam*.25,seam/*TIMING_INSERT_16_BEGIN*/,timingCompositeObserver/*TIMING_INSERT_16_END*/);/*TIMING_INSERT_15_BEGIN*/timingEnd('submission');/*TIMING_INSERT_15_END*/renderReady=true;frames++;
