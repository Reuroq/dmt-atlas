...
from field_centre_v33r3 import field, parts, compose_jets, lower_parts, cheap_parts

HERE = Path(__file__).resolve().parent

def main():
    destination = HERE/'numeric-candidate-v33r3-check.json'
    ...
    with sync_playwright() as p:
        ...
        page.goto((HERE/'gpu_probe_v33r3_candidate.html').as_uri())
        ...
        leaf_page.goto((HERE/'gpu_probe_v33r3_leaves.html').as_uri())
        for w,h,times in [(48,32,[4,4.895,8]), (49,33,[4,4.895,8,221,900])]:
            for z in [8,-6]:
                for t in times:
                    for high in [1,0]:
        ...
        browser.close()
    (HERE/'numeric-candidate-v33r3-raw.json').write_text(json.dumps(cases)+'\n')
    ...
    for case in cases:
        ...
        lower_keys=['axA','axB','angA','angB','shellAp','shellAn','shellBp','shellBn']
        ...
        cheap_jets=np.stack([np.array(case['leaves'][key]).reshape(-1,4) for key in ['cheapF','cheapA','cheapB']],axis=1)
    ...
    out = {'method':'New v33r3 actual float GLSL rays plus all smooth-leaf jets at saved actual roots. Independent float64 scalar/leaf FD; exact float32 CSG selection and original normal replay. Composite FD disagreements retained at creases, not a unique-normal assertion. Original six plus central3x3 dense .003 first-crossing references, even/odd grids, HIGH/LOW, entry/deep, times4/4.895/8/221/900. Sampled evidence only; no universal convergence, filtering, realism or performance proof.',
           'limits':{'gradient':.005,'bound_ratio':1.001,'scalar':.001,'root':.03},
           'files':{name:digest(HERE/name) for name in ['continuum.js','trip.js','continuum-v25.js','continuum-v33r3-candidate.js','build_centre_v33r3.py','build_centre_v33r3_checks.py','gpu_probe_v33r3_leaves.html','gpu_probe_v33r3_candidate.html','probe_numeric_candidate_v33r3.py','field_centre_v33r3.py','numeric-candidate-v33r3-raw.json']},
    ...
    out['passed'] = not errors and all(s['passed'] for s in summaries) and len(root_errors)==32*15
