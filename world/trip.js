...
// reports/depictions live in data-reports.js so they are not on the critical path.
// Fetched on idle after the first frame; the drawer awaits it only if a reader is
// faster than the network. window.WORLD_REPORTS is the loaded payload.
let trailPromise=null;
function loadTrail(){
 if(window.WORLD_REPORTS)return Promise.resolve(window.WORLD_REPORTS);
 if(!trailPromise)trailPromise=new Promise(resolve=>{
  const s=document.createElement('script');s.src='data-reports.js';
  s.onload=()=>resolve(window.WORLD_REPORTS);
  s.onerror=()=>{window.WORLD_REPORTS={reports:{},depictions:{},failed:true};resolve(window.WORLD_REPORTS);};
  document.head.appendChild(s);});
 return trailPromise;
}
(window.requestIdleCallback||(f=>setTimeout(f,1200)))(()=>loadTrail());
