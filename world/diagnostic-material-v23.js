  ...
  fragmentShader:`
   ...
   void main(){
    ...
    if(hit){
     ...
     if(panel>1.5&&panel<2.5 || panel>3.5)recess=1.;
     ...
     if(panel<2.5)color+=mix(pigment,vec3(1.),.18)*pow(max(0.,dot(reflect(rd,normal),normalize(vec3(.3,.8,.6)))),10.)*.18*recess;
     if(panel>4.5)color=radiance(reflect(rd,normal));
