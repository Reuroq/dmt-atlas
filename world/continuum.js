  ...
  fragmentShader:`
   ...
   float field(vec3 p){
    ...
    for(int j=0;j<7;j++){
     ...
     if(high>.5)wave+=.035*sin(phase*4.-radial*.64-opening*2.);
     ...
     float sheet=abs(p.z-sheetZ)/4.5-.075;
   ...
   void main(){
    ...
    if(hit){
     ...
     float hue=.18*sin(a*3.+time*.12)+p.z*.018+time*.025;
