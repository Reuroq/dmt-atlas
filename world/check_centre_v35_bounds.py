"""ONE-SHOT prospective v35 bounds/clearance/attachment fixture; not yet run.
...
import numpy as np
from field_centre_v35 import PIXEL, smooth_parts, expanded_leaves, compose_expanded, field, analytic_bounds
from csg_interval_v35 import contract_checks

HERE = Path(__file__).resolve().parent
OUT = HERE/'centre-v35-bounds-check.json'
START = HERE/'centre-v35-bounds-started.json'
FAILURE = HERE/'centre-v35-bounds-failure.json'
...
def main():
    ...
    gate = json.loads((HERE/'centre-v35-fixture-integrity.json').read_text())
    ...
    save(START, {'started': True, 'fixture_manifest': digest('centre-v35-fixture-integrity.json')})
    ...
    out = {'passed': passed, 'cases': results, 'clearance': clearance, 'cap_join': cap_checks,
           ...
           'source_hashes': {name: digest(name) for name in [Path(__file__).name,
               'field_centre_v35.py', 'csg_interval_v35.py', 'continuum-v35-candidate.js',
               'centre-v35-bound-design.json', 'centre-v35-fixture-integrity.json', START.name]},
