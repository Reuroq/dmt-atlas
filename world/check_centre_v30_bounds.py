"""One-shot independent v30 leaf, reach, clearance and footprint samples."""
import hashlib
import json
from pathlib import Path
import numpy as np
from field_centre_v30 import PIXEL,parts,field
from fidelity import RENDER_FILES

HERE=Path(__file__).resolve().parent
OUT=HERE/'centre-v30-bounds-check.json'
...
for pixel in [PIXEL,PIXEL*2,PIXEL*4]:
    L,M=13+12*pixel,160+250*pixel+600*pixel**2
    for z in [8,-6]:
        ...
        for high in [1,0]:
            ...
            petals=np.maximum(reach(f,df),np.maximum(reach(abs(a)-.19,np.sign(a)*da),
                np.minimum(reach(.10+b,db),reach(.10-b,-db))))
            branches=np.maximum(reach(f+.32,df),np.maximum(reach(abs(a)-.40,np.sign(a)*da),reach(abs(b)-.055,np.sign(b)*db)))
            step=np.minimum(np.minimum(petals,branches),reach(f+9,df))
...
inner=np.column_stack([5.479*np.cos(theta),1.7+5.479*np.sin(theta),rng.uniform(-27.5,8,n)])
clearance=[]
for points,label in [(corridor,'walk corridor'),(inner,'rho5.479 before cap')]:
...
# Symbolic recurrence arithmetic, independently recorded with coefficients.
Ls,Hs=[],[]
for k,b,c in [(.46,.28,0),(1.25,.72,1),(3.4,1.45,.9),(9.1,2.8,.8)]:
    L=k+b+c*(Ls[-1] if Ls else 0);H=L*L+c*(Hs[-1] if Hs else 0)
    Ls.append(L);Hs.append(H)
b_hessian=[Hs[2]+.24*Hs[3],2*np.exp(-.5)*(Ls[2]*10+.24*Ls[3]*26),100+.24*26**2]
assert all(a<b for a,b in zip(b_hessian,[160,250,600]))
after={n:digest(n) for n in protected}
files=['continuum-v30-candidate.js','build_centre_candidate_v30.py','build_centre_v30_checks.py',
    'field_centre_v30.py','centre-v30-bounds.md',Path(__file__).name,
    'gpu_probe_v30_candidate.html','gpu_probe_v30_leaves.html','probe_numeric_candidate_v30.py']
out={'passed':all(r['passed'] for r in results+clearance+filter_checks) and before==after,
    ...
    'recurrence_L':Ls,'recurrence_H':Hs,'b_H_polynomial':b_hessian,
...
print(json.dumps({'passed':out['passed'],'leaf_samples':len(results)*n,
    ...
    'recurrence_H':Hs,'b_H_polynomial':b_hessian,'clearance':clearance}))
assert out['passed']
