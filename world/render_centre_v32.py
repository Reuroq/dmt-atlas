...
HTML = PREFIX+'.html'
CANDIDATE = 'continuum-v32-candidate.js'
assert not list(HERE.glob(PREFIX+'*')), 'Preserve one-shot evidence'
gate=json.loads((HERE/'centre-v32-integrity.json').read_text())
...
html = html.replace('A reported-pattern interpretation', 'ISOLATED v32 · synthesised diagnostic')
