...
OUT=HERE/'diagnostic-numeric-v25.json'
...
with sync_playwright() as p:
    ...
    page.goto((HERE/'diagnostic_numeric_v25.html').as_uri())
    for case in cases:
    ...
    browser.close()
out={'method':'Four saved worst-position float32 points, actual archived GLSL intermediates, then replace only atan(y,x) with a CPU-computed float32 angle. No navigation/time/visual acceptance claims.',
     'errors':errors,'cases':results,'sha256':{name:digest(HERE/name) for name in ['continuum.js','continuum-v25.js','trip.js','diagnostic_numeric_v25.html','diagnose_numeric_v25.py','continuum-v25-numerical-findings.json']}}
