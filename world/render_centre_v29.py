...
PREFIX = 'diagnostic-centre-v29'
HTML = PREFIX+'.html'
CANDIDATE = 'continuum-v29-candidate.js'
assert not list(HERE.glob(PREFIX+'*')), 'Preserve one-shot evidence'
gate=json.loads((HERE/'centre-v29-integrity.json').read_text())
...
html = html.replace('A reported-pattern interpretation', 'ISOLATED v29 · synthesised diagnostic')
