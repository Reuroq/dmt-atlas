...
HISTORY = 'centre-v34-transport-runtime-integrity.json'
SEEDS = 'centre-v34-followup-seeds.json'
TEXTURE = 'centre-v34-followup-texture.f32'
HTML = 'gpu_centre_v34_staged_arithmetic_r3.html'
...
def receipt(label):
    ...
    assert launch['script_sha256'] == digest(launch['script'])
    assert launch['launcher_sha256'] == digest('run_centre_v34_staged_arithmetic_r3_once.py')
...
def assess(r, leaves, texels, readbacks):
    ...
    if op == 'float-extension' and not v['supported']:
        return result('fatal-backend')
    if op == 'dispose' and not cleanup_valid(v, r.get('resources')):
        return result('fatal-disposal')
    if op == 'shared' and (not exact_words(s.get('dataBits'), source_bits()) or v['input']['values'] != texels
...
def main():
            ...
            try:
                ...
                if page is not None:
                    try:
                        ...
                        prior_steps = [read(n)['payload'] for n in events if read(n)['kind'] == 'step-result']
                        ...
                        if a['classification'] != 'ok':
                            cleanup_failure = failure = 'Resource disposal failed; see durable evidence'
                    except Exception as exc:
                        event('dispose-failure', {'error': str(exc)}); cleanup_failure = failure = 'Resource disposal incomplete'
                    try:
    ...
    save(PREFIX + '-result.json', {'complete': complete, 'infrastructure_clean': clean, 'failure': failure,
         'cleanup_failure': cleanup_failure, 'browser_messages': messages, 'assessments': assessments, 'events': {n: digest(n) for n in events},
