...
PREFIX = 'diagnostic-static-compositor-v18'
...
receipt = {'purpose':'Static diagnostic, not live acceptance or correction',
...
with sync_playwright() as p:
    ...
    for mode in ['present','absent']:
        ...
        if mode=='present':page.screenshot(path=str(HERE/f'{PREFIX}.png'))
        page.close()
    browser.close()
receipt['protected_unchanged']={name:digest(HERE/name)==h for name,h in baseline.items()}
receipt['artifact_hashes']={name:digest(HERE/name) for name in [Path(__file__).name,f'{PREFIX}.html',f'{PREFIX}.js',f'{PREFIX}.png']}
...
print('Static isolation completed; no live runtime changes.',flush=True)
