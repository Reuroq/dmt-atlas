...
import numpy as np
from field_centre_v33r3 import PIXEL,parts

HERE=Path(__file__).resolve().parent
OUT=HERE/'centre-v33r3-first-root-check.json'
assert not OUT.exists()
def digest(name):
    return hashlib.sha256((HERE/name).read_bytes()).hexdigest()
baseline=json.loads((HERE/'numeric-candidate-v33r3-check.json').read_text())
assert all(digest(n)==h for n,h in baseline['files'].items())
bounds=json.loads((HERE/'centre-v33r3-bounds-check.json').read_text())
assert bounds['passed'] and all(digest(n)==h for n,h in bounds['source_hashes'].items())
cases=json.loads((HERE/'numeric-candidate-v33r3-raw.json').read_text())
...
def lower_bound(left,right,width):
    ...
    # The v33r3 union/intersection is monotone in all three smooth leaves.
...
files=['numeric-candidate-v33r3-check.json','numeric-candidate-v33r3-raw.json',
    'centre-v33r3-bounds-check.json',
    'field_centre_v33r3.py',Path(__file__).name]
out={'passed':other_gpu_checks and all(c['passed'] for c in results) and len(flat)==480,
