...
import subprocess
import tempfile
from static_centre_v34_staged_arithmetic_r3 import durable_node, staged_checks
...
def main():
    ...
    from checks_centre_v34_staged_arithmetic_r3 import check_changes, extra_checks
    check_changes()
    ...
    durable_node('syntax', js, syntax=True)
    node = r'''
...
'''.replace('SOURCE', json.dumps(js), 1)
    result = staged_checks(js, node); plan = result['plan']
    ...
    def good(step, values=None, value=None):
        return {'step': step, 'before': {'errors': [], 'drained': True}, 'after': {'errors': [], 'drained': True},
                'skipped': False, 'contextLost': False, 'exception': None, 'resources': [],
    ...
    import run_centre_v34_staged_arithmetic_r3_once as launcher
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='staged-arithmetic-static-',dir=p.HERE) as temp:
            ...
            for name in (launcher.PathName,'probe_centre_v34_staged_arithmetic_r3.py',p.PREFIX+'-freeze.json',p.PREFIX+'-preparation-integrity.json'):
