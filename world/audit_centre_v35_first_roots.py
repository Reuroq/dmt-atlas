...
import numpy as np
from field_centre_v35 import PIXEL,smooth_parts,expanded_leaves,compose_expanded
from csg_interval_v35 import enclosure
from runtime_centre_v35 import fixture_gate,one_shot
from cost_lifecycle_v35 import atomic_json

HERE=Path(__file__).resolve().parent
OUT=HERE/'centre-v35-first-root-check.json'
assert not OUT.exists()
def digest(name):
    return hashlib.sha256((HERE/name).read_bytes()).hexdigest()
def main():
    baseline=json.loads((HERE/'numeric-candidate-v35-check.json').read_text())
    assert baseline['passed'] and not baseline['errors']
    assert all(digest(n)==h for n,h in baseline['files'].items())
    bounds=json.loads((HERE/'centre-v35-bounds-check.json').read_text())
    assert bounds['passed'] and all(digest(n)==h for n,h in bounds['source_hashes'].items())
    cases=json.loads((HERE/'numeric-candidate-v35-raw.json').read_text())
    design=json.loads((HERE/'centre-v35-bound-design.json').read_text())
    ...
    other_gpu_checks=baseline['passed'] and not baseline['errors'] and all(c['misses']==0 and c['max_all_leaf_gradient_error']<.005
    ...
    files=['numeric-candidate-v35-check.json','numeric-candidate-v35-raw.json',
        'centre-v35-bounds-check.json',
        'field_centre_v35.py','csg_interval_v35.py','centre-v35-bound-design.json',
        'runtime_centre_v35.py','cost_lifecycle_v35.py','centre-v35-fixture-integrity.json',
        'centre-v35-first-root-started.json',Path(__file__).name]
    out={'passed':other_gpu_checks and all(c['passed'] for c in results) and len(flat)==720,
        ...
        'method':'Independent float64 CSG interval exclusion expanding five bases to eleven signed leaves before endpoint extrema and smooth-leaf linear-interpolation error M*h²/8; envelope excludes singular axis. Refine earliest potential interval left-first to actual positive/negative bracket <=1e-7. Original .003 grid result retained separately. No GPU replay, changed tolerance, forced root or minimum step.',
