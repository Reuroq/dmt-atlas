...
import subprocess
import sys
from probe_centre_v34_staged_arithmetic_r2 import HERE, PREFIX, digest, gate, save
...
def main(label):
    ...
    for suffix in ('-' + label + '-once-launch.json', '-' + label + '-once-exit.json', '-' + label + '-once.log',
        ...
        assert not (HERE / (PREFIX + suffix)).exists(), 'Never replay'
    script = {'static': 'check_centre_v34_staged_arithmetic_r2.py', 'close': 'close_centre_v34_staged_arithmetic_r2_preparation.py',
              'runtime': 'probe_centre_v34_staged_arithmetic_r2.py'}[label]
...
PathName = 'run_centre_v34_staged_arithmetic_r2_once.py'
