"""One-shot independent v31 leaf, reach, clearance and footprint samples."""
import hashlib
import json
from pathlib import Path
import numpy as np
from field_centre_v31 import PIXEL,parts,field,analytic_bounds
from fidelity import RENDER_FILES

HERE=Path(__file__).resolve().parent
OUT=HERE/'centre-v31-bounds-check.json'
...
inner=np.column_stack([4.899*np.cos(theta),1.7+4.899*np.sin(theta),rng.uniform(-27.5,8,n)])
clearance=[]
for points,label in [(corridor,'walk corridor'),(inner,'rho4.899 before cap')]:
...
files=['continuum-v31-candidate.js','build_centre_candidate_v31.py','build_centre_v31_checks.py',
    'field_centre_v31.py','centre-v31-bounds.md',Path(__file__).name,
    'gpu_probe_v31_candidate.html','gpu_probe_v31_leaves.html','probe_numeric_candidate_v31.py']
