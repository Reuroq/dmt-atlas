...
'''.replace('SOURCE', json.dumps(js), 1)
    ...
    import run_centre_v34_staged_arithmetic_r2_once as launcher
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='staged-arithmetic-static-',dir=p.HERE) as temp:
            ...
            for name in (launcher.PathName,'probe_centre_v34_staged_arithmetic_r2.py',p.PREFIX+'-freeze.json',p.PREFIX+'-preparation-integrity.json'):
