...
import numpy as np
from field_centre_v32 import PIXEL,parts

HERE=Path(__file__).resolve().parent
OUT=HERE/'centre-v32-first-root-check.json'
assert not OUT.exists()
def digest(name):
    return hashlib.sha256((HERE/name).read_bytes()).hexdigest()
baseline=json.loads((HERE/'numeric-candidate-v32-check.json').read_text())
assert all(digest(n)==h for n,h in baseline['files'].items())
bounds=json.loads((HERE/'centre-v32-bounds-check.json').read_text())
assert bounds['passed'] and all(digest(n)==h for n,h in bounds['source_hashes'].items())
cases=json.loads((HERE/'numeric-candidate-v32-raw.json').read_text())
M=305+190*PIXEL+110*PIXEL**2

def composed(v):
    f,a,b=np.moveaxis(v,-1,0)
    return np.minimum(np.minimum(np.maximum(f,np.maximum(abs(a)-.10,.035-abs(b))),
        np.maximum(f+.38,np.maximum(abs(a)-.25,abs(b)-.014))),f+10)
...
def lower_bound(left,right,width):
    ...
    return np.minimum(np.minimum(np.maximum(fl,np.maximum(al-.10,.035-bu)),
        np.maximum(fl+.38,np.maximum(al-.25,bl-.014))),fl+10)
...
files=['numeric-candidate-v32-check.json','numeric-candidate-v32-raw.json',
    'centre-v32-bounds-check.json',
    'field_centre_v32.py',Path(__file__).name]
