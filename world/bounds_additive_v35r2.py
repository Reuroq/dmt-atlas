"""ONE-SHOT prospective v35r2 bounds/clearance/attachment fixture; not yet run.
...
import numpy as np
from field_centre_v35r2 import PIXEL, smooth_parts, expanded_leaves, compose_expanded, field, analytic_bounds
from csg_interval_v35r2 import contract_checks

HERE = Path(__file__).resolve().parent
OUT = HERE/'centre-v35r2-additive-bounds/result.json'
START = HERE/'centre-v35r2-additive-bounds/bounds-started.json'
FAILURE = HERE/'centre-v35r2-additive-bounds/bounds-failure.json'
...
def main():
    ...
    from additive_runtime_v35r2 import fixture_gate
    gate = fixture_gate()
    before = {name: digest(name) for name in gate['protected']}
    save(START, {'started': True, 'fixture_manifest': digest('centre-v35r2-fixture-integrity.json')})
    ...
    out = {'passed': passed, 'cases': results, 'clearance': clearance, 'cap_join': cap_checks,
           ...
           'source_hashes': {name: digest(name) for name in [Path(__file__).name,
               'field_centre_v35r2.py', 'csg_interval_v35r2.py', 'continuum-v35r2-candidate.js',
               'centre-v35r2-bound-design.json', 'centre-v35r2-fixture-integrity.json', str(START.relative_to(HERE)), 'additive_runtime_v35r2.py']},
